...
{
  options.components.sw.remoteBuild = lib.mkOption {
  ...
  config = {
    components.sw.remoteBuild.enable = lib.mkDefault (config.components.sw.type == "tablet-kiosk");
    ...
    systemd.services.update-system = {
      ...
      serviceConfig = {
        ...
        ExecStart = 
          let
            hosts = config.components.sw.remoteBuild.hosts;
            ...
            remoteBuildFlags = if config.components.sw.remoteBuild.enable
              then
                ''--builders "${builders}"''
              else "";
          in
