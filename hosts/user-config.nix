    ...
    users.users =
      let
        enabledAccounts = lib.filterAttrs (
          name: _: lib.elem name config.components.users.enabledUsers
        ) config.components.users.accounts;
      in
      lib.mapAttrs (
        name: user:
        let
        ...
        in
        {
          ...
          shell = if user.shell != null then user.shell else config.components.users.shell;
        }
      ) enabledAccounts;
